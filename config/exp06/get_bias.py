...
relation_dir_path = os.path.join(ROOT_DIR, '../dataset/Structured3D/meta/relation_depth')
savefile_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'bias_depth.txt')
num_classes = 41
        ...
        if f.split('.')[-1] == 'json':
            with open(os.path.join(roots, f), 'r') as fin:
                ...
                for i in range(num_classes):
                    ...
                    rel_table[curr_cate, i] = sum(dis_list) / len(dis_list) if len(dis_list) > 0 and curr_cate != i else 0    # 同类则对bias无贡献
                    dist_sum += rel_table[curr_cate, i]
                for i in range(num_classes):
                    rel_table[curr_cate, i] = rel_table[curr_cate, i] / (dist_sum + 1.)
