...
def display_instances_second_class(image, boxes, masks, class_ids, second_class_ids, class_names,
    ...
    for i in range(N):
        ...
        if not captions:
            class_id = class_ids[i]
            second_class_id = second_class_ids[i]
            score = scores[i] if scores is not None else None
            label = class_names[class_id]
            score2 = second_scores[i] if second_scores is not None else None
            label2 = class_names[second_class_id]
            if second_class_id != 0:
                caption = "{} {:.3f}/{} {:.3f}".format(label, score, label2, score2) if score else label
            else:
                caption = "{} {:.3f}".format(label, score) if score else label
        else:
            caption = captions[i]
